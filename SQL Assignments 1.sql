...
SELECT AVG(Length) AS AverageSongLength
FROM Songs;
...
WHERE Length = (
    SELECT MAX(Length)
    FROM Songs AS s
    WHERE songs.ID = Songs.ID
);

12. Get the number of songs for each band

SELECT Bands.Name AS BandName, COUNT(Songs.ID) AS SongCount
FROM Bands
JOIN Albums ON Bands.ID = Albums.ID
